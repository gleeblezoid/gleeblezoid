...
The UI opening step is only seen on the client side - monitoring the success of this process by actively having telemetry from the client would increase performance overhead on our app and also introduce complex problems related to processing data about the environment which are beyond our control and may not even be relevant to a given failure. Users may also be unhappy about the performance impact and the increased demand for sharing data. 
...
Client side monitoring can still be used to great effect in situations where all the information required is present within our own application and generating and sending such data would have little performance impact, we’ll see some of these later on. Examples of this include the response codes from `BillingClient.BillingResponseCode` with the exceptions of `SERVICE_DISCONNECTED`, `SERVICE_UNAVAILABLE`, `SERVICE_TIMEOUT`, and `USER_CANCELED` which are all either non-specific in terms of root cause or explicitly caused by the user/network connection. 

There's no location on the original briefing for this exercise where information from clients is collected so to include SLIs which would use such data I’ll use "our logging server" to refer to the point of measurement and disambiguate from the "server" shown in the original setup. I'm working under the assumption that we have an EULA and Privacy agreements set up already to allow the collection of such data.
...
I've chosen a time threshold of 1000ms as that's an industry average (see https://cloud.google.com/architecture/adopting-slos#latency_alerts) for read operations and fits with our average 0.1-1 transactions per second being served.
...
Once a user has been presented with the store and chosen to purchase more in-game currency they start the purchase flow. If this request isn't successful then that will make our users unhappy so we should measure the availability of this flow.
...
This doesn’t quite give us the full picture, it's great for us to know about if something does go wrong, but what users actually experience is more related to them seeing purchases turn up on their account than the verification of their purchase token. 
...
This is distinct from standard integration testing as we're not simply checking that the code in our purchase process works, we're also looking into the effects of factors which will impact our customer base around the world and how to tailor our SLIs to those variables. If our servers are located only in the US, for example, then customers in EMEA will experience a higher level of latency than our US customers which we can only improve by setting up in an additional region. We'll need to adjust our service or our SLIs based on what makes sense for us at the time, especially given the additional complexity that expanding to multiple regions can introduce.

If we're seeing more traffic than usual then our previously less relevant checks on Purchase Tokens may be more necessary as additional stress is applied to the database on our server. We may need to monitor the throughput and coverage on our `Update Account` service as well in case the larger volume of requests reveals limitations in our setup which cause waiting periods that make users unhappy or even result in data processing failures when they try to purchase our in-game currency.

We can also look at data from the Play Store on a regular cadence to monitor user happiness in the form of crashes and reviews for the application as a whole. If we're seeing crash events or unfavourable reviews related to our in-game store and in-game currency purchasing then that might indicate we've missed the mark on some of our SLIs in terms of what users really care about. We should be cautious, especially with reviews, to ensure we're processing that information in a way that gets to the heart of what users find frustrating. For example, the cost of our in-game currency is not something that we, as SREs, should deal with.

In conclusion, I've tried to select a few relevant components of the user journey which can be quantitatively measured and relate directly to the user's experience of trying to buy in-game currency in our app. I’ve not covered parts of the user journey which would require extensive extrapolation from the exercise material or major changes to implement SLIs (namely the initial presentation of the UI).

In prioritizing user happiness and where we can improve reliability based on SLOs I've tried to stay away from using indicators which depend more on the performance of a 3rd party (generally the Play Store or user's network provider) or implementations which require intrusive data gathering on the client device. 

By selecting SLIs in this way we can calculate meaningful error budgets which allow us to improve the purchasing experience for users looking to get some more in-game currency via ongoing development.
